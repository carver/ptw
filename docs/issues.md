...
2. **systemd user services only get the groups you had at login.** Joining
   `input` needs a logout before `systemctl --user start ptw` can read the
   keyboard. `ptw doctor` (`service` line) and the daemon now say so. A
   `.deb` could ship the udev rule and a login hook; for now it is a
   documented step.
3. **The Key proxy has not run on a real keyboard.** ADR 0006. Things
...
4. **Deferral is a constant (150 ms).** Make it a config knob if anyone
   needs a different trade-off between Alt+click and slow chord presses.
5. **Thread count on the host.** In the sandbox 2 threads beat 8 by
   1.7–1.9x, which is surprising enough to re-measure with real use.
   Default stays 2 until then.

## Correction and hold-back

6. **Multi-word names the model splits further.** "Bernal Heights" came
...
7. **Thresholds are set from five recordings.** ACCEPT_BELOW 0.15 and the
...
8. **Names still wrong on the samples:** a neighbourhood (sometimes),
   a first name, "ptw".

## Engine

9. **160 ms lookahead stays opt-in.** It costs 2–2.5x CPU and the commit
   lag gain is under half a second. Document in the settings dialog when
   a low-latency mode is worth it.
10. **sherpa-onnx as a second Engine** once its hotwords PR
   (k2-fsa/sherpa-onnx#3895) merges: model-side biasing for custom words
   would beat post-hoc Correction.
11. **Moonshine v2** only if a faster x86 build appears; 2–4x slower and
    less accurate on our samples (see `docs/research/engine-benchmark.md`).
12. **Benchmark reference transcripts** in `target/bench/` were not
    proofread; WER numbers in the benchmark report are approximate.

## Typing and hotkey

13. **Per-character portal latency.** Each keysym is a D-Bus round trip.
    If it shows in use, batch through libei or type words per call.
14. **Chord keys pressed in the wrong order leak.** Pressing z before Alt
    types a z, because only modifier presses are deferred (ADR 0006).
15. **Toggle mode has no UI.** `ptw toggle` over D-Bus works; nothing
    exposes it.

## Packaging

16. **`.deb` package** so setup is `apt install` plus `ptw setup`.
17. **Hotkey capture in the settings dialog.** Today you type the chord
