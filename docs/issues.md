...
1. **Audio cues untested on the host.** Everything else on the first
   host pass is verified (2026-08-29): hotkey, capture, portal typing,
   streaming, the tray's red disc, the settings dialog at its new size,
   and the systemd unit after the `input`-group relogin.
2. **The Key proxy has only done a plain Hold on a real keyboard.**
   ADR 0006. Still to watch for: keys stuck down after start or quit, the
   Deferral biting Alt+click, autorepeat behaviour of forwarded keys, a
   second keyboard plugged in while running, and whether `ptw setup`'s
...
15. **`.deb` package** so setup is `apt install` plus `ptw setup`. It
