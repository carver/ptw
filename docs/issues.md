...
3. **Portal reconnect after suspend is untested on the host.** Suspend
   invalidated the portal session on 2026-08-30 and every chunk was
   dropped with "Invalid session" until a daemon restart. The session
   loop now reconnects the Typist and retypes the failed chunk (tested
   in ptw-core); still to check on the host: suspend, resume, dictate,
   and confirm the journal shows one "reconnecting the Typist" and the
   text lands.
