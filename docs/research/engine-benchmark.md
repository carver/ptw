...
- **More threads are slower on this box.** 8 threads is 1.7 to 1.9x slower than 2 for both Nemotron runtimes; 4 is in between. *Superseded on the host, 2026-08-29:* `ptw transcribe` on sample 3 ran 3.2x real time at 1 thread, 5.6x at 2, 6.0x at 4, 6.7x at 6, 7.4x at 8 and 3.9x at 0 (all 16 hardware threads). The sandbox result was its CPU quota. Default stays 2 as the cheapest comfortably-real-time count (ADR 0005).
