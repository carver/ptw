...
- Reading does not consume the key, so the Hotkey also reaches the focused app. ~~We accept this and choose a default Hotkey that is harmless in common apps, rather than building a grab-and-reemit keyboard proxy.~~ Superseded by ADR 0006: the held modifier turned typed text into shortcuts, so ptw grabs the keyboard after all.
