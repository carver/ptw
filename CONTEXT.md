...
**Hold-back**:
Committed words withheld from typing until enough following words have arrived to decide whether a Correction applies to them. As long as the longest Custom word, plus one when that is a phrase, because the Engine splits words it does not know.
